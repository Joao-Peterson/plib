...
db_results_t *db_exec(db_t *db, char *query, size_t params_count, ...){
	...
	if(retries == 0 && conn == NULL){
		va_end(params);
		return db_results_new_fmt(db->vendor, 0, 0, db_error_fatal, "Could not get connnection from connection pool. Connection available: [%lu]. Connection count: [%lu]", db->context.available_connection, db->context.connections_count);
	}
